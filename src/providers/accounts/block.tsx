...
export const SolarweaveDatabase = 'solarweave-cache-devnet-testrun4';
...
export async function fetchBlock(dispatch: Dispatch, url: string, cluster: Cluster, key: string, type: string) {
  dispatch({
  ...
  try {
    if (type === 'slot') {
      result = await RetrieveSlot(key, SolarweaveDatabase);
    } else {
      result = await RetrieveBlockhash(key, SolarweaveDatabase);
    }
...
export function useFetchBlock() {
  ...
  return React.useCallback((key: string, type: string) => {
      ...
      if (!entry) {
        fetchBlock(dispatch, url, cluster, key, type);
      }
    },
